	...
	public void notify(DelegateTask delegateTask) {
		...
		if (EVENTNAME_COMPLETE.equals(eventName)) {
			...
			if (FALSE.equalsIgnoreCase(delegateTask.getVariable("approved").toString())) {
				...
				if (delegateTask.getVariable(ZHILIANG) != null && delegateTask.getVariable(SHIGONG) != null
						&& delegateTask.getVariable(ANQUAN) != null
						&& TRUE.equals(delegateTask.getVariable(ZHILIANG).toString())
			...
			baseService.sendSteamWeChat("projectVolume", "工程量上报流程变更通知",
					baseService.getWeChatDesc(TASKMAP.get(taskDefinitionKey), taskName,
							TRUE.equalsIgnoreCase(delegateTask.getVariable("approved").toString()) ? "通过" : "不通过",
									suggestion));
