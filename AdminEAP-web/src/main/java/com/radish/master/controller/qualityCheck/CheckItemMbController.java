 ...
 */
@Controller
@RequestMapping("/checkRecordMb")
public class CheckItemMbController {
	...
	private static final String uploaderPath=PropertiesUtil.getValue("qualityChecksFilePath")+"\\checkRecordMb";
    ...
    public String list() {
        ...
        return "QualityChecks/checkRecordMb/check_tree";
    }
    @RequestMapping(method = RequestMethod.GET, value = "/list")
    public String listIndex(HttpServletRequest request) {
    	String lx = request.getParameter("lx");
		request.setAttribute("lx",lx);
        return "QualityChecks/checkRecordMb/addIndex";
    }
    ...
    public String addWj(HttpServletRequest request) {
        ...
        return "QualityChecks/checkRecordMb/edit";
    }
    ...
    public String scWj(HttpServletRequest request) {
        ...
        return "QualityChecks/checkRecordMb/query_file";
    }
    
    @RequestMapping("saveWj")
    @ResponseBody
    public Result saveWj(HttpServletRequest request,CheckRecordMb cr){
    	Result r = new Result();
    	String fid = request.getParameter("fid");
    	...
    	if(cr.getId()==null){
    		...
    		CheckRecordMb c = baseService.get(CheckRecordMb.class, cr.getId());
    ...
    public List<CheckItem> getAll() {
        ...
        return checkItemService.find(hql.toString());
    }
    ...
    public List<TreeNode> getTreeData() {
        return checkItemService.getTreeData();
    }

    @RequestMapping(value = "/get/{id}", method = RequestMethod.POST)
    @ResponseBody
    public CheckItem get(@PathVariable("id") String id) {
    	CheckItem mat = checkItemService.get(CheckItem.class, id);
        if (!StrUtil.isEmpty(mat.getParentId())) {
        	mat.setParentName(checkItemService.get(CheckItem.class, mat.getParentId()).getName());
        } else {
    ...
    public Result show(@PathVariable("id") String id) {
        return checkItemService.getMatNames(id);
    }

    @RequestMapping(value = "/getList/{code}", method = RequestMethod.POST)
    @ResponseBody
    public List<CheckItem> getMatsByCode(String code) {
        return checkItemService.getMatsByCode(code);
    }

    @RequestMapping(value = "/save", method = RequestMethod.POST)
    @ResponseBody
    public Result save(CheckItem org) {

        org.setUpdateDateTime(new Date());
        checkItemService.saveOrUpdate(org);
        if (!StrUtil.isEmpty(org.getParentId())) {
        	CheckItem parent = checkItemService.get(CheckItem.class, org.getParentId());
        	checkItemService.deleteCacheByKey(RedisConstant.CHECK_PRE + parent.getCode());
        }
        checkItemService.deleteCacheByKey(RedisConstant.CHECK_PRE + "tree");
    ...
    public Result delete(@PathVariable("id") String id) {
        ...
        try {
        	CheckItem org = checkItemService.get(CheckItem.class, id);
            if(checkItemService.referByMaterial(org.getId())) {
               return new Result(false,"该物料分类被物料表引用，不可删除");
            }
            checkItemService.delete(org);
            if (!StrUtil.isEmpty(org.getParentId())) {
            	CheckItem parent = checkItemService.get(CheckItem.class, org.getParentId());
            	checkItemService.deleteCacheByKey(RedisConstant.MAT_PRE + parent.getCode());
            }
            checkItemService.deleteCacheByKey(RedisConstant.CHECK_PRE + "tree");
            return new Result(true);
        } catch (Exception e) {
    ...
    public Result deleteWj(String id,HttpServletRequest request) {
    	...
    	CheckRecordMb cr = baseService.get(CheckRecordMb.class, id);
	...
	public String fileIndex(HttpServletRequest request){
		...
		return "QualityChecks/checkRecordMb/fileIndex";
	}
