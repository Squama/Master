...
public class PurchaseServiceImpl extends BaseServiceImpl implements PurchaseService {
	...
	public List<Options> getProjectCombobox() {
		return this.findMapBySql("select id value, project_name data from tbl_project", new Object[] {},
				new Type[] { StringType.INSTANCE }, Options.class);
	}

	@Override
	public List<Options> getBudgetCombobox() {
		return this.findMapBySql("select budget_no value, budget_name data from tbl_budget", new Object[] {},
				new Type[] { StringType.INSTANCE }, Options.class);
	}
	...
	public List<Options> getBudgetComboboxByProject(String projectID) {
		return this.findMapBySql(
				"select budget_no value, budget_name data from tbl_budget where project_id=? AND status=30",
	...
	public List<Options> getRegionComboboxByBudgetNo(String budgetNo) {
		return this.findMapBySql(
				"select region_code value, region_name data from tbl_budget_tx where budget_no=? and region_code is not null",
	...
	public MatMap getMatComboboxByRegion(String budgetNo, String regionID) {
		List<Materiel> list = this.findMapBySql(
				...
				new Object[] { budgetNo, regionID }, new Type[] { StringType.INSTANCE, StringType.INSTANCE },
				Materiel.class);
	...
	public Dispatch getDispatchByProAndPur(String sourceProjectID, String targetProjectID, String purchaseID) {
		...
		return this.get(
				"from Dispatch where sourceProjectID=:sourceProjectID AND targetProjectID=:targetProjectID AND purchaseID=:purchaseID",
				params);
	}
	...
	public List<DispatchDetail> getDispatchDetailList(String dispatchID, String purchaseDetID) {
		...
		return this.find("from DispatchDetail where dispatchID = :dispatchID AND purchaseDetID = : purchaseDetID",
				params);
	}
	...
	public PurchaseDet getPurchaseDetOri(String purchaseID, String regionID, String MatNumber) {
		...
		return this.get(
				"from PurchaseDet where purchaseID=:purchaseID AND regionID=:regionID AND MatNumber=:MatNumber AND stockType <> '2'",
				params);
	}
	...
	public void executeDispatch(String projectID, String id, String matID, String num) {
		...
		Dispatch dispatch = this.getDispatchByProAndPur(projectID, purchase.getProjectID(),
				purchaseDetOld.getPurchaseID());
		if (dispatch == null) {
	...
	public void cancelDispatch(String id) {
		...
		Dispatch dispatch = this.getDispatchByProAndPur("402880e860c947ea0160ca0239670000", purchase.getProjectID(),
				purchaseDetOld.getPurchaseID());
		...
		if (stockService.thawStockChannel(list)) {
			// 加回原来的
			PurchaseDet detOriginal = this.getPurchaseDetOri(purchaseDetOld.getPurchaseID(),
					purchaseDetOld.getRegionID(), purchaseDetOld.getMatNumber());
	...
	public List<PurchaseApplyAudit> getQuantityListMap(String purchaseID) {
		...
		sb.append(
				"SELECT PUR.id, PUR.budget_no, ORI.region_name, PUR.mat_number, PUR.mat_name, PUR.mat_standard, ORI.budget, COST.cost, PUR.apply FROM ");
		sb.append(
				"(SELECT PD.id,P.budget_no, PD.region_id, PD.mat_number, PD.mat_name, PD.mat_standard, SUM(PD.quantity) apply ");
		...
		sb.append("LEFT JOIN");
		sb.append(
				"(SELECT BT.budget_no, BT.region_code, BT.region_name, BE.mat_number, BE.mat_name, BE.mat_standard, BE.quantity budget ");
		sb.append("FROM tbl_budget_tx BT, tbl_budget_estimate BE ");
		sb.append("WHERE BT.id = BE.budget_tx_id) ORI ");
		sb.append(
				"ON PUR.budget_no = ORI.budget_no AND PUR.region_id = ORI.region_code AND PUR.mat_number = ORI.mat_number ");
		sb.append("LEFT JOIN");
		sb.append(
				"(SELECT P.budget_no, PD.region_id, PD.mat_number, PD.mat_name, PD.mat_standard, SUM(PD.quantity) cost ");
		sb.append("FROM tbl_purchase P, tbl_purchase_det PD ");
		sb.append("WHERE PD.purchase_id = P.id AND P.id != ? ");
		sb.append("GROUP BY P.budget_no, PD.mat_number, PD.mat_name, PD.mat_standard, PD.region_id) COST ");
		sb.append(
				"ON COST.budget_no = ORI.budget_no AND COST.region_id = ORI.region_code AND COST.mat_number = ORI.mat_number ");
